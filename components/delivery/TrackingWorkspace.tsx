...
export function TrackingWorkspace({ token }: { token: string }) {
  const [tracking, setTracking] = useState<Tracking | null>(null)
  useEffect(() => {
    fetch('/api/delivery/tracking?token=' + encodeURIComponent(token))
      .then(async response => setTracking(await response.json()))
      .catch(() => setTracking({ error: 'Tracking is unavailable.' }))
  }, [token])
