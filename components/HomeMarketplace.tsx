  ...
  return <section className="home-marketplace" aria-labelledby="marketplace-title">
    <div className="shell">
      <div className="marketplace-intro">
        <div><span className="kicker">Module marketplace</span><h2 id="marketplace-title">Choose your first app.<br />Add the rest when ready.</h2></div>
        <p>Each module works independently and connects automatically. Activate one today, switch on another next month, and keep one subscription across your business.</p>
      </div>
      <div className="storefront-grid">
        {homepageModules.map(({ id, shortName, description, price, usagePricing, icon: Icon, category }) => {
          ...
          return <article className={active ? 'storefront-card selected' : 'storefront-card'} key={id}>
            ...
            <div className="storefront-price"><strong>£{price}</strong><span>/ month<small>{usagePricing}</small></span></div>
