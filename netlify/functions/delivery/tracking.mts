...
export default async (request: Request, _context: Context) => {
  ...
  const now = new Date()
  const [tracking] = await db.select().from(deliveryTrackingTokens).where(and(eq(deliveryTrackingTokens.tokenHash, tokenHash), isNull(deliveryTrackingTokens.revokedAt), or(isNull(deliveryTrackingTokens.expiresAt), gt(deliveryTrackingTokens.expiresAt, now)))).limit(1)
